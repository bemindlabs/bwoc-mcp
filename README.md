...
**v1.1.0.** Both transports (**stdio** + **Streamable HTTP** with bearer auth),
the **full 24-tool catalog** (read open; write / exec / lifecycle gated), and
MCP **resources** (`bwoc://agents|fleet|info`) + **prompts** (`delegate`,
`fleet_review`) are shipped and tested (7 integration tests, clippy clean).
...
Desktop extension** (`.mcpb` bundle, attached to each release). See the
