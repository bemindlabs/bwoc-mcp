...
**v1.2.0.** Both transports (**stdio** + **Streamable HTTP** with bearer auth),
the **full 24-tool catalog** (read open; write / exec / lifecycle gated), and
MCP **resources** (`bwoc://agents|fleet|info`) + **prompts** (`delegate`,
`fleet_review`) are shipped and tested (clippy clean), and a nightly CLI
contract test checks every tool against the latest bwoc release.
v1.1.0 adds **per-tool MCP annotations** on every tool (read-only / destructive /
open-world hints for hosts + the Connectors Directory) and a **one-click Claude
Desktop extension** (`.mcpb` bundle, attached to each release). v1.1.2 made
`bwoc_fleet` / `bwoc://fleet` work on bwoc 3.x (`fleet health`); v1.2.0 moves to
rmcp 3.4, clearing three MCP SDK advisories (two high, on the HTTP transport). See the
[changelog](CHANGELOG.md) and [`docs/PLAN.md`](docs/PLAN.md).
